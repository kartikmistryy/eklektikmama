...
export function createTicketQRData(transactionId, ticketNumber = null, eventTitle = null, eventLocation = null, eventDate = null, eventTime = null, eventSegment = null, eventMessage = null, eventMeetingLink = null) {
  ...
  // For Eklektik Edit events, QR code should point to ticket details page (not QR page)
  if (eventSegment === 'eklektikEdit') {
    return `${baseUrl}/ticket/${transactionId}`;
  }
  
  // For other event segments, QR code points to the QR page with all details
...
export function createQRCodeSection(qrCodeDataUrl, transactionId, ticketNumber = null, ticketUrl = null, eventTitle = null, eventLocation = null, eventDate = null, eventTime = null, eventSegment = null) {
    ...
    <div style="text-align: center; margin: 20px 0; background: white; padding: 20px; border-radius: 8px; border: 1px solid #e0e0e0;">
      ...
      <div style="margin: 20px 0;">
        <a href="${qrPageUrl}" 
          ...
          ${eventSegment === 'eklektikEdit' ? '📱 View Your Event Details & Meeting Link' : '📱 View Your QR Code & Event Details'}
        </a>
      </div>
      
      <p style="margin: 10px 0; font-size: 14px; color: #666;">
        ${eventSegment === 'eklektikEdit' 
          ? 'Click the button above to view your event details and meeting link' 
      ...
      <div style="margin-top: 15px; font-size: 12px; color: #6c757d; background: #f8f9fa; padding: 10px; border-radius: 4px; text-align: left;">
        <strong>${eventSegment === 'eklektikEdit' ? 'How to Join Your Session:' : 'Alternative Entry Methods:'}</strong><br>
        ${eventSegment === 'eklektikEdit' ? (
          `• Use the meeting link provided in your event details<br/>
