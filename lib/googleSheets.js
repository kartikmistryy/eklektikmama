...
export const createEventSheet = async (eventData) => {
  try {
    ...
    if (!sheet) {
      ...
      // Add extra guest columns at the end (for all segments)
      headers.push(
        'Extra Guest Names',
        'Extra Guest Emails',
...
export const addBookingToEventSheet = async (bookingData, eventData) => {
  try {
    ...
    if (eventData.segment === 'cinemaMorning') {
      ...
      // Debug main course selection - this should go in Dietary Requirements for now
      console.log('🍽️ Main Course Selection Debug:', {
        mainCourseSelection: bookingData.mainCourseSelection,
        choiceI: bookingData.choiceI,
        dietaryRequirements: bookingData.dietaryRequirements,
        finalValue: bookingData.mainCourseSelection || bookingData.choiceI || ''
      });
      
      // Calculate main course selection value
      ...
      // Festive Mornings has same fields as mamaBreakfast but without menu choices
      rowData['Child Age'] = bookingData.childAge || '';
      rowData['Child Gender'] = bookingData.childGender || '';
      rowData['Child Date of Birth'] = bookingData.childDob || '';
      rowData['Dietary Requirements'] = bookingData.dietaryRequirements || '';
