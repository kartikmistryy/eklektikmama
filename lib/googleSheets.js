  ...
  try {
    ...
    if (!sheet) {
      ...
      if (totalColumns > 26) { // Google Sheets default is 26 columns
        await sheet.resize({ rowCount: 1000, colCount: Math.max(totalColumns, 30) });
      }
      
      // Set the headers for column mapping
      await sheet.setHeaderRow(headers);
      
      // Note: We don't add a header row as data because setHeaderRow() already sets the headers
      // The headers will be available for column mapping but won't appear in getRows()
...
export const addBookingToEventSheet = async (bookingData, eventData) => {
  try {
    ...
    // Ensure the sheet has proper headers before adding data
    const existingRows = await sheet.getRows();
    
    // If the sheet is empty or doesn't have proper headers, log a warning
    if (existingRows.length === 0) {
      console.log('⚠️  Warning: Sheet appears to be empty');
    } else if (existingRows[0]._rawData[0] !== 'Booking Date/Time') {
      console.log('⚠️  Warning: Sheet may not have proper headers');
      console.log('   - First row data:', existingRows[0]._rawData);
    } else {
      console.log('✅ Sheet has proper headers, adding booking data');
    }
    ...
    // Add the row and verify it was added correctly
    const newRow = await sheet.addRow(rowData);
    console.log(`✅ Booking added to event sheet: ${sheetName}`);
    console.log(`   - Row added at index: ${newRow.rowIndex || 'unknown'}`);
