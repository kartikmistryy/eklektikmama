...
export const getMembersSheet = async () => {
...
export const addMemberToSheet = async (membershipData) => {
  try {
    console.log('=== ADDING NEW MEMBER TO GOOGLE SHEETS ===');
    console.log('Member Email:', membershipData.email);
    console.log('Member Name:', `${membershipData.firstName} ${membershipData.lastName}`);
    console.log('Membership Type:', membershipData.membershipType);
    console.log('Status:', membershipData.status);
    ...
    const rowData = {
      ...
      'Plan Type': membershipData.membershipType === 'monthly' ? 'Monthly' : 'Annual', // Transform to proper case
    ...
    console.log('Row Data to be added:', JSON.stringify(rowData, null, 2));
    ...
    await logGoogleSheetsOperation({
      ...
      recordId: nextRowId.toString(),
      recordEmail: membershipData.email,
      newValues: rowData,
      source: 'api',
      sourceDetails: 'addMemberToSheet function'
    });
    
    console.log('✅ Member successfully added to Google Sheets with Row ID:', nextRowId);
    console.log('=== END ADDING MEMBER ===\n');
    
    return nextRowId;
  } catch (error) {
    console.error('❌ Error adding member to Google Sheets:', error);
    console.error('Member data that failed:', JSON.stringify(membershipData, null, 2));
    throw error;
  }
};
...
export const updateMemberInSheet = async (email, updateData) => {
  try {
    console.log('=== UPDATING MEMBER IN GOOGLE SHEETS ===');
    ...
    if (!memberRow) {
      console.error('❌ Member not found in Google Sheets:', email);
      throw new Error(`Member with email ${email} not found in Google Sheets`);
    }
    
    console.log('Found member row with ID:', memberRow.get('Row ID'));
    
    // Store original values for comparison
    const originalValues = {};
    const changes = {};
    
    // Update the row data and track changes
    Object.keys(updateData).forEach(key => {
      if (updateData[key] !== undefined) {
        const originalValue = memberRow.get(key);
        ...
        if (originalValue !== newValue) {
          ...
          console.log(`📝 Field "${key}" changed: "${originalValue}" → "${newValue}"`);
        } else {
          console.log(`➡️ Field "${key}" unchanged: "${originalValue}"`);
        }
        
        memberRow.set(key, newValue);
      }
    });
    ...
    if (Object.keys(changes).length > 0) {
      ...
      console.log('ℹ️ No changes detected - all values were the same');
    }
    ...
    await logGoogleSheetsOperation({
      ...
      sheetName: 'Members',
      spreadsheetId: process.env.GOOGLE_SHEETS_SPREADSHEET_ID,
      recordId: memberRow.get('Row ID'),
      recordEmail: email,
      oldValues: originalValues,
      newValues: updateData,
      changes: changes,
      source: 'api',
      sourceDetails: 'updateMemberInSheet function'
    });
    
    console.log('✅ Member successfully updated in Google Sheets');
    console.log('=== END UPDATING MEMBER ===\n');
...
export const getMemberFromSheet = async (email) => {
  try {
    ...
    return {
      ...
      membershipType: memberRow.get('Plan Type'),
...
export const getAllMembersFromSheet = async () => {
  try {
    ...
    return rows.map(row => ({
      ...
      phone: row.get('Phone'),
      membershipType: row.get('Plan Type'),
...
export const addContactFormToSheet = async (formData, source) => {
  try {
    ...
    const rowData = {
      ...
      'Idea Details': formData.ideaDetails || ''
    };
...
export const createEventSheet = async (eventData) => {
  try {
    ...
    if (!sheet) {
      ...
      } else if (eventData.segment === 'festiveMornings') {
        headers.push(
          'Child Age',
          'Child Gender',
          'Child Date of Birth',
      ...
      } else if (eventData.segment === 'coffeeMeetup') {
        headers.push(
          'Child Name',
          'Child Age',
          'Membership Status',
      ...
      // Add extra guest columns (for all segments)
      headers.push(
        'Extra Guest Names',
        'Extra Guest Emails',
        'Extra Guest Menu Selections'
      );
      
      // Add menu selection columns at the end if event has dynamic menu selections (for mamaBreakfast and festiveMornings)
      if ((eventData.segment === 'mamaBreakfast' || eventData.segment === 'festiveMornings') &&
          eventData.menuSelections && Array.isArray(eventData.menuSelections) && eventData.menuSelections.length > 0) {
        eventData.menuSelections.forEach((menuSelection, index) => {
          if (menuSelection && menuSelection.label) {
            headers.push(`Menu Selection ${index + 1} (${menuSelection.label})`);
          }
        });
      }
...
export const addBookingToEventSheet = async (bookingData, eventData) => {
  try {
    ...
    if (!sheet) {
      ...
      const createResult = await createEventSheet(eventData);
      ...
      // Wait a bit more for the sheet to be fully ready
    ...
    // For Coffee Meetup events, use the correct field names
    console.log('🔍 Segment Debug - Event segment:', eventData.segment);
    console.log('🔍 Segment Debug - Is coffee meetup?', eventData.segment === 'coffeeMeetup');
    
    if (eventData.segment === 'coffeeMeetup') {
      console.log('🔍 Coffee Meetup Debug - Raw booking data:', {
      ...
      rowData['Guardian Name'] = bookingData.name || bookingData.guardianName || '';
      rowData['Child Name'] = bookingData.childName || '';
      rowData['Phone'] = bookingData.phone || '';
      
      console.log('🔍 Coffee Meetup Debug - Mapped row data:', {
        guardianName: rowData['Guardian Name'],
        childName: rowData['Child Name'],
        phone: rowData['Phone']
      });
    }
    ...
    if (eventData.segment === 'cinemaMorning') {
      ...
      // Debug all mama breakfast data
      console.log('🍽️ Mama Breakfast Data Debug:', {
        ...
        foodAllergies: bookingData.foodAllergies,
        ...
        mainCourseSelection: bookingData.mainCourseSelection,
        choiceI: bookingData.choiceI,
        choiceII: bookingData.choiceII,
        choiceIII: bookingData.choiceIII
      });
      
      // Calculate main course selection value (for backward compatibility)
      const mainCourseValue = bookingData.mainCourseSelection || bookingData.choiceI || bookingData.dietaryRequirements || '';
      console.log('🍽️ Setting Dietary Requirements to:', mainCourseValue);
      
      rowData['Child Age'] = bookingData.childAge || '';
      rowData['Child Gender'] = bookingData.childGender || '';
      // Skip Child Date of Birth - it doesn't exist in the actual sheet
      rowData['Dietary Requirements'] = mainCourseValue;
      rowData['Emergency Contact'] = bookingData.emergencyName || '';
      rowData['Emergency Phone'] = bookingData.emergencyPhone || '';
      // Map allergies to Medical Conditions since there's no separate Allergies column
      rowData['Medical Conditions'] = bookingData.allergies || bookingData.medicalConditions || '';
      rowData['Photography Consent'] = bookingData.photographyConsent || 'No';
      rowData['Special Requests'] = bookingData.specialRequests || '';
      rowData['Table Preferences'] = bookingData.tablePreferences || '';
      // Map notes to Additional Notes
      rowData['Additional Notes'] = bookingData.notes || bookingData.additionalNotes || '';
      
      // Add menu selections to separate columns if event has dynamic menu selections
      if (eventData.menuSelections && Array.isArray(eventData.menuSelections) && eventData.menuSelections.length > 0) {
        eventData.menuSelections.forEach((menuSelection, index) => {
          if (menuSelection && menuSelection.label) {
            const fieldName = index === 0 ? 'choiceI' : index === 1 ? 'choiceII' : 'choiceIII';
            const columnName = `Menu Selection ${index + 1} (${menuSelection.label})`;
            rowData[columnName] = bookingData[fieldName] || '';
            console.log(`🍽️ Menu Selection ${index + 1} (${menuSelection.label}):`, bookingData[fieldName] || '');
          }
        });
      }
    } else if (eventData.segment === 'festiveMornings') {
      // Festive Mornings has same fields as mamaBreakfast
      rowData['Child Age'] = bookingData.childAge || '';
      rowData['Child Gender'] = bookingData.childGender || '';
      rowData['Child Date of Birth'] = bookingData.childDob || '';
      rowData['Dietary Requirements'] = bookingData.dietaryRequirements || '';
      ...
      if (eventData.menuSelections && Array.isArray(eventData.menuSelections) && eventData.menuSelections.length > 0) {
        eventData.menuSelections.forEach((menuSelection, index) => {
          if (menuSelection && menuSelection.label) {
            ...
            const columnName = `Menu Selection ${index + 1} (${menuSelection.label})`;
            rowData[columnName] = bookingData[fieldName] || '';
            console.log(`🎉 Menu Selection ${index + 1} (${menuSelection.label}):`, bookingData[fieldName] || '');
          }
        });
      }
    } else if (eventData.segment === 'mamaFit') {
      rowData['Ticket Type'] = bookingData.ticketType || '';
      rowData['Fitness Level'] = bookingData.fitnessLevel || '';
      rowData['Medical Clearance'] = bookingData.medicalClearance || '';
      rowData['Medical Conditions'] = bookingData.medicalConditions || '';
      ...
      rowData['Child Name'] = bookingData.childName || '';
      rowData['Child Age'] = bookingData.childAge || '';
      rowData['Membership Status'] = 'Active Member';
      rowData['Photography Consent'] = bookingData.photographyConsent || 'No';
    }
    
    // Add extra guest data (comma-separated)
    // Use the structured extra object if available, otherwise fall back to comma-separated strings
    if (bookingData.extra && bookingData.extra.name && bookingData.extra.name.length > 0) {
      rowData['Extra Guest Names'] = bookingData.extra.name.join(', ');
      rowData['Extra Guest Emails'] = bookingData.extra.email ? bookingData.extra.email.join(', ') : '';
      rowData['Extra Guest Menu Selections'] = bookingData.extra.menuSelections ? bookingData.extra.menuSelections.join(', ') : '';
    } else if (bookingData.extraGuestNames) {
      rowData['Extra Guest Names'] = bookingData.extraGuestNames || '';
      rowData['Extra Guest Emails'] = bookingData.extraGuestEmails || '';
      rowData['Extra Guest Menu Selections'] = bookingData.extraGuestMainCourses || '';
    } else {
      rowData['Extra Guest Names'] = '';
      rowData['Extra Guest Emails'] = '';
      rowData['Extra Guest Menu Selections'] = '';
    }
    ...
    // Convert rowData object to array in the correct order - MUST match actual Google Sheet headers
    let headers = [
    ...
    // Add segment-specific headers - MUST match actual Google Sheet column order
    if (eventData.segment === 'cinemaMorning') {
      headers.push(
      ...
      // CRITICAL: This must match the actual Google Sheet headers exactly
      // NOTE: The actual sheet is missing "Child Date of Birth" column!
      headers.push(
        'Child Age', 'Child Gender', 'Dietary Requirements',
        'Emergency Contact', 'Emergency Phone', 'Medical Conditions', 'Photography Consent',
        'Special Requests', 'Table Preferences', 'Additional Notes'
      );
    } else if (eventData.segment === 'festiveMornings') {
      // Festive Mornings has same headers as mamaBreakfast
      headers.push(
        'Child Age', 'Child Gender', 'Child Date of Birth',
        'Dietary Requirements', 'Emergency Contact', 'Emergency Phone', 'Medical Conditions', 'Photography Consent',
        'Special Requests', 'Table Preferences', 'Additional Notes'
      );
    } else if (eventData.segment === 'mamaFit') {
      headers.push(
        'Ticket Type', 'Fitness Level', 'Pregnant', 'Postpartum', 'Postpartum Duration',
      ...
      headers.push(
        'Child Name', 'Child Age', 'Membership Status', 'Photography Consent'
      );
    }
    ...
    // Debug the row data mapping
    console.log('🔍 Row Data Mapping Debug:');
    console.log('Headers:', headers);
    console.log('RowData keys:', Object.keys(rowData));
    console.log('RowData values:', Object.values(rowData));
    ...
    // Calculate the last column based on headers length
    const numToCol = (num) => {
      let col = '';
      while (num > 0) {
    ...
    const lastColumnLetter = numToCol(headers.length);
    const response = await sheets.spreadsheets.values.update({
      spreadsheetId: doc.spreadsheetId,
      range: `${sheetName}!A${nextRow}:${lastColumnLetter}${nextRow}`,
...
export const addBookingToGeneralSheet = async (bookingData, eventData) => {
  try {
    ...
    const rowData = {
      ...
      'Medical Clearance': bookingData.medicalClearance || '',
      ...
      'Extra Guest Names': (bookingData.extra && bookingData.extra.name) ? bookingData.extra.name.join(', ') : (bookingData.extraGuestNames || ''),
      'Extra Guest Emails': (bookingData.extra && bookingData.extra.email) ? bookingData.extra.email.join(', ') : (bookingData.extraGuestEmails || ''),
      'Extra Guest Menu Selections': (bookingData.extra && bookingData.extra.menuSelections) ? bookingData.extra.menuSelections.join(', ') : (bookingData.extraGuestMainCourses || '')
    };
...
export const getEventBookingsCount = async (eventData) => {
  try {
    ...
    // Use direct Google Sheets API to get data more reliably
    const serviceAccountAuth = new JWT({
      email: process.env.GOOGLE_SHEETS_CLIENT_EMAIL,
      key: process.env.GOOGLE_SHEETS_PRIVATE_KEY?.replace(/\\n/g, '\n'),
      scopes: ['https://www.googleapis.com/auth/spreadsheets'],
    ...
    console.log(`Found ${allRows.length} total rows in sheet (including header)`);
    ...
    dataRows.forEach((row, index) => {
      console.log(`Processing data row ${index + 1}:`, row);
      
      // "Number of Tickets" should be in column 8 (index 7)
      const tickets = parseInt(row[7]) || 0;
      
      if (!isNaN(tickets) && tickets > 0) {
        totalTickets += tickets;
        console.log(`✅ Row ${index + 1}: Added ${tickets} tickets (Total: ${totalTickets})`);
      } else {
        console.log(`❌ Row ${index + 1}: Invalid ticket count (${tickets}), row data:`, row);
      }
    });
