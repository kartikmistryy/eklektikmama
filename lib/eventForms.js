  ...
  cinemaMorning: {
    ...
    fields: [
      ...
      // Friends & Family Discount Section
      {
        ...
        type: "checkbox",
        text: "Bring a guest and save 10%! Each ticket you purchase can include one guest ticket (for another mama, friend, or family member).You’ll pay for both tickets together and receive 10% off the total."
      },
      {
  ...
  mamaFit: {
    ...
    fields: [
      // Ticket Type Selection
      {
        ...
        type: "radio",
        options: ["Solo mum (no sitter) - 115 AED", "Mum + Baby (with sitters) - 155 AED"],
        required: true,
      },
      // Mother's Details
      { name: "motherName", label: "Mother's Full Name", type: "text", required: true },
      ...
      // Child's Details (optional - only shown for Mum + Baby ticket type)
      { name: "childName", label: "Child's Full Name", type: "text", required: false },
      { name: "childDob", label: "Child's Date of Birth", type: "date", required: false },
      { name: "childAge", label: "Child's Age", type: "number", required: false },
...
I release and discharge Eklektik Mama, its instructors, and event partners from any claims, liabilities, or demands that may arise in connection with my participation.
  ...
  festiveMornings: {
    ...
    fields: [
      ...
      // Family Portraits Note
      {
        name: "familyPortraitsNote",
        label: "Family Portraits",
        type: "info",
        text: "*Please note: Family portraits are available to purchase directly from Krisz Meloni. Tickets are final sale, non-transferable, and valid only for this event.*"
      },
...
export const spreadsheetIds = {
  ...
  festiveMornings: process.env.FESTIVE_MORNINGS_SPREADSHEET_ID,
...
export const getFormBySegment = (segment, eventData = null) => {
  const segmentKey = segment?.toLowerCase()?.replace(/\s+/g, '') || '';
  
  if (segmentKey.includes('cinema')) {
    return eventForms.cinemaMorning;
  } else if (segmentKey.includes('breakfast')) {
    // For mamaBreakfast, use dynamic menu selections if available
    const breakfastForm = { ...eventForms.mamaBreakfast };
    
    console.log('🍽️ Processing mamaBreakfast form:', {
      hasMenuSelections: !!eventData?.menuSelections,
      menuSelectionsCount: eventData?.menuSelections?.length || 0,
    ...
    if (eventData?.menuSelections && Array.isArray(eventData.menuSelections) && eventData.menuSelections.length > 0) {
      // Create a deep copy of fields array to avoid mutating the original
      breakfastForm.fields = [...breakfastForm.fields];
      
      console.log('🍽️ Before processing - fields:', breakfastForm.fields.map(f => f.name));
      
      // Find the index where to insert menu selections (after notes, before Friends & Family section)
      const notesIndex = breakfastForm.fields.findIndex(f => f.name === 'notes');
      const friendsFamilyIndex = breakfastForm.fields.findIndex(f => f.name === 'applyFriendsFamilyDiscount');
      
      // Remove old static menu selection fields if they exist
      const choiceIIndex = breakfastForm.fields.findIndex(f => f.name === 'choiceI');
      const beverageNoteIndex = breakfastForm.fields.findIndex(f => f.name === 'beverageNote');
      
      console.log('🍽️ Found indices:', {
        notesIndex,
        friendsFamilyIndex,
        choiceIIndex,
        beverageNoteIndex
      ...
      // Remove static fields in reverse order to maintain indices
      if (beverageNoteIndex !== -1) {
        breakfastForm.fields.splice(beverageNoteIndex, 1);
      }
      if (choiceIIndex !== -1) {
        breakfastForm.fields.splice(choiceIIndex, 1);
      }
      
      // Insert dynamic menu selections after notes, before Friends & Family section
      // Calculate insert index: after notes, or before friendsFamilyIndex if it exists
      const insertIndex = friendsFamilyIndex !== -1 ? friendsFamilyIndex : (notesIndex !== -1 ? notesIndex + 1 : breakfastForm.fields.length);
      
      // Build menu fields to insert
      const menuFieldsToInsert = [];
      eventData.menuSelections.forEach((menuSelection, index) => {
        if (menuSelection && menuSelection.label && menuSelection.options && Array.isArray(menuSelection.options) && menuSelection.options.length > 0) {
          const fieldName = index === 0 ? 'choiceI' : index === 1 ? 'choiceII' : 'choiceIII';
          menuFieldsToInsert.push({
            name: fieldName,
      ...
      console.log('🍽️ Menu fields to insert:', menuFieldsToInsert.map(f => ({ name: f.name, label: f.label, optionsCount: f.options.length })));
      
      // Insert all menu fields at once
      if (menuFieldsToInsert.length > 0) {
        breakfastForm.fields.splice(insertIndex, 0, ...menuFieldsToInsert);
      }
      
      console.log('🍽️ After processing - fields:', breakfastForm.fields.map(f => f.name));
      console.log('🍽️ Mama Breakfast Dynamic Menu Selections:', {
        menuSelections: eventData.menuSelections,
        insertedFields: menuFieldsToInsert,
        totalFields: breakfastForm.fields.length
      });
    } else {
      console.log('🍽️ No dynamic menu selections found, using static form');
    }
    
    return breakfastForm;
  } else if (segmentKey.includes('festive')) {
    // For festiveMornings, use dynamic menu selections if available
    const festiveForm = { ...eventForms.festiveMornings };
    
    console.log('🎉 Processing festiveMornings form:', {
      hasMenuSelections: !!eventData?.menuSelections,
      menuSelectionsCount: eventData?.menuSelections?.length || 0,
      menuSelections: eventData?.menuSelections
    });
    ...
    if (eventData?.menuSelections && Array.isArray(eventData.menuSelections) && eventData.menuSelections.length > 0) {
      // Create a deep copy of fields array to avoid mutating the original
      festiveForm.fields = [...festiveForm.fields];
      
      console.log('🎉 Before processing - fields:', festiveForm.fields.map(f => f.name));
      
      // Find the index where to insert menu selection (after notes, before family portraits)
      const notesIndex = festiveForm.fields.findIndex(f => f.name === 'notes');
      const familyPortraitsIndex = festiveForm.fields.findIndex(f => f.name === 'familyPortraitsNote');
      
      // Remove old static menu selection fields if they exist
      const choiceIIndex = festiveForm.fields.findIndex(f => f.name === 'choiceI');
      const beverageNoteIndex = festiveForm.fields.findIndex(f => f.name === 'beverageNote');
      
      console.log('🎉 Found indices:', {
        notesIndex,
        familyPortraitsIndex,
        choiceIIndex,
        beverageNoteIndex
      });
      ...
      // Remove static fields in reverse order to maintain indices
      if (beverageNoteIndex !== -1) {
        festiveForm.fields.splice(beverageNoteIndex, 1);
      }
      if (choiceIIndex !== -1) {
        festiveForm.fields.splice(choiceIIndex, 1);
      }
      
      // Insert dynamic menu selections after notes, before family portraits
      const insertIndex = familyPortraitsIndex !== -1 ? familyPortraitsIndex : (notesIndex !== -1 ? notesIndex + 1 : festiveForm.fields.length);
      ...
      console.log('🎉 Menu fields to insert:', menuFieldsToInsert.map(f => ({ name: f.name, label: f.label, optionsCount: f.options.length })));
      
      // Insert all menu fields at once
      if (menuFieldsToInsert.length > 0) {
        festiveForm.fields.splice(insertIndex, 0, ...menuFieldsToInsert);
      }
      
      console.log('🎉 After processing - fields:', festiveForm.fields.map(f => f.name));
      console.log('🎉 Festive Mornings Dynamic Menu Selections:', {
        menuSelections: eventData.menuSelections,
        insertedFields: menuFieldsToInsert,
        totalFields: festiveForm.fields.length
      });
    } else if (eventData?.hasMenuSelection) {
      // Legacy support: use static menu selection if hasMenuSelection is true but no dynamic selections
      const notesIndex = festiveForm.fields.findIndex(f => f.name === 'notes');
      if (notesIndex !== -1) {
        festiveForm.fields.splice(notesIndex + 1, 0, {
          name: "choiceI",
          label: "Main Course Selection",
          type: "select",
          options: [
