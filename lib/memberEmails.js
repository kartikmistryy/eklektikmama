...
// Membership upgrade confirmation email
export const sendMembershipUpgradeEmail = async (memberData, upgradeDetails) => {
  try {
    const { firstName, lastName, email } = memberData;
    
    // Import the upgrade email template
    const { generateMembershipUpgradeEmail } = await import('./emailTemplates.js');
    
    // Generate the upgrade confirmation email
    const emailContent = generateMembershipUpgradeEmail(firstName, email, upgradeDetails);
    
    // Send the email using the existing service
    const result = await sendEmailViaService(email, emailContent);
    console.log('Membership upgrade email sent:', result.messageId);
    return result;
    
  } catch (error) {
    console.error('Error sending membership upgrade email:', error);
    throw error;
  }
};

// Membership expiration reminder email
export const sendMembershipExpirationReminderEmail = async (memberData, membershipDetails) => {
  try {
    const { firstName, lastName, email } = memberData;
    
    // Import the expiration reminder email template
    const { generateMembershipExpirationReminderEmail } = await import('./emailTemplates.js');
    
    // Generate the expiration reminder email
    const emailContent = generateMembershipExpirationReminderEmail(firstName, email, membershipDetails);
    
    // Send the email using the existing service
    const result = await sendEmailViaService(email, emailContent);
    console.log('Membership expiration reminder email sent:', result.messageId);
    return result;
    
  } catch (error) {
    console.error('Error sending membership expiration reminder email:', error);
    throw error;
  }
};
...
export const sendCancellationConfirmationEmail = async (memberData) => {
  try {
    ...
    // Debug logging
    console.log('🔍 Email function received data:', {
      firstName,
      email,
      membershipType,
      currentPeriodEnd,
    ...
    const subject = `Your Eklektik AF Membership Has Been Cancelled`;
    ...
    const emailContent = {
      ...
      <head>
        <meta charset="utf-8">
        <title>Your Eklektik AF Membership Has Been Cancelled</title>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { background: linear-gradient(135deg, #093166 0%, #DB4E9F 100%); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0; }
          .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
          .info-box { background: #f0f9ff; border: 1px solid #0ea5e9; padding: 20px; margin: 20px 0; border-radius: 8px; }
          .refund-box { background: #f0fdf4; border: 1px solid #22c55e; padding: 20px; margin: 20px 0; border-radius: 8px; }
          .feedback-box { background: #fefce8; border: 1px solid #eab308; padding: 20px; margin: 20px 0; border-radius: 8px; }
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>Your Eklektik AF Membership Has Been Cancelled</h1>
          </div>
          
          <div class="content">
            <h2>Hi ${firstName},</h2>
            
            <p>This is a quick note to confirm your Eklektik AF membership has been cancelled.</p>
            
            <div class="info-box">
              <h3>📅 Your Access Continues</h3>
              <p>You'll still have access to all your perks until <strong>${(() => {
                if (!currentPeriodEnd) return 'the end of your current billing period';
                try {
                  const date = new Date(currentPeriodEnd);
                  if (isNaN(date.getTime())) return 'the end of your current billing period';
                  return date.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
                } catch (error) {
                  console.error('Date formatting error in email:', error);
                  return 'the end of your current billing period';
                }
              })()}</strong>:</p>
              <ul>
                <li><strong>Monthly memberships</strong> stay active until the last day of the month you've already paid for.</li>
                <li><strong>Annual memberships</strong> stay active until the last day of your paid year.</li>
              </ul>
              <p>After that, your discounts, early access, and other member benefits will simply come to an end.</p>
            </div>
            
            <div class="refund-box">
              <h3>💰 Refund Information</h3>
              <p>If you cancelled within 7 days of signing up, you may be eligible for a refund. If so, we'll process it and let you know once it's on the way.</p>
            </div>
            
            <div class="feedback-box">
              <h3>💬 We'd Love Your Feedback</h3>
              <p>If there's anything we could have done differently — or if you'd like to share why you cancelled — we'd genuinely love to hear from you. Just reply to this email or reach us at <a href="mailto:hello@eklektikmama.com" style="color: #DB4E9F;">hello@eklektikmama.com</a>.</p>
            </div>
            
            <div class="divider">⸻</div>
            
            <p style="text-align: center; font-style: italic; color: #666;">
              Thank you for being part of the Eklektik Mama community. Whether you're with us as a member or just popping in for events, we'll always be here, cheering you on and raising a little hell together.
            </p>
            
            <p style="text-align: center; margin-top: 30px;">
              <strong>With gratitude,<br>Team Eklektik Mama</strong>
            </p>
          </div>
        </div>
      </body>
    ...
    `,
      text_content: `
Your Eklektik AF Membership Has Been Cancelled

Hi ${firstName},

This is a quick note to confirm your Eklektik AF membership has been cancelled.

YOUR ACCESS CONTINUES:
You'll still have access to all your perks until ${(() => {
  if (!currentPeriodEnd) return 'the end of your current billing period';
  try {
    const date = new Date(currentPeriodEnd);
    if (isNaN(date.getTime())) return 'the end of your current billing period';
    ...
    console.error('Date formatting error in email text:', error);
    return 'the end of your current billing period';
  }
})()}:
• Monthly memberships stay active until the last day of the month you've already paid for.
• Annual memberships stay active until the last day of your paid year.

After that, your discounts, early access, and other member benefits will simply come to an end.

REFUND INFORMATION:
If you cancelled within 7 days of signing up, you may be eligible for a refund. If so, we'll process it and let you know once it's on the way.

WE'D LOVE YOUR FEEDBACK:
If there's anything we could have done differently — or if you'd like to share why you cancelled — we'd genuinely love to hear from you. Just reply to this email or reach us at hello@eklektikmama.com.
...
// Membership upgrade confirmation email
export const sendUpgradeConfirmationEmail = async (upgradeData) => {
  try {
    const { email, firstName, monthlyExpiryDate, annualStartDate } = upgradeData;
    
    const subject = `Membership Upgrade Confirmed - Welcome to Annual!`;
    ...
    const emailContent = {
      ...
      <head>
        <meta charset="utf-8">
        <title>Membership Upgrade Confirmed</title>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { background: linear-gradient(135deg, #093166 0%, #DB4E9F 100%); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0; }
          .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
          .upgrade-box { background: #f0fdf4; border: 2px solid #22c55e; padding: 20px; margin: 20px 0; border-radius: 8px; }
          .timeline-box { background: #fefce8; border: 1px solid #eab308; padding: 20px; margin: 20px 0; border-radius: 8px; }
          .benefits-box { background: #f0f9ff; border: 1px solid #0ea5e9; padding: 20px; margin: 20px 0; border-radius: 8px; }
          .cta-button { display: inline-block; background: #093166; color: white; padding: 15px 30px; text-decoration: none; border-radius: 25px; margin: 20px 0; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            ...
            <p>Welcome to Annual Membership</p>
          </div>
          
          <div class="content">
            <h2>Hi ${firstName},</h2>
            
            <p>Great news! Your membership upgrade to annual has been successfully processed. You're now set to enjoy even more savings and benefits!</p>
            
            <div class="upgrade-box">
              <h3>✅ Upgrade Details</h3>
              <p><strong>Current Status:</strong> Monthly membership active until ${new Date(monthlyExpiryDate).toLocaleDateString()}</p>
              <p><strong>Annual Membership Starts:</strong> ${new Date(annualStartDate).toLocaleDateString()}</p>
              <p><strong>Savings:</strong> You'll save 2 months worth of membership fees!</p>
            </div>
            
            <div class="timeline-box">
              <h3>📅 What Happens Next</h3>
              <ul>
                <li><strong>Right now:</strong> Continue enjoying your monthly membership benefits</li>
                <li><strong>On ${new Date(monthlyExpiryDate).toLocaleDateString()}:</strong> Your annual membership automatically begins</li>
                <li><strong>Next payment:</strong> ${new Date(new Date(annualStartDate).getTime() + 365 * 24 * 60 * 60 * 1000).toLocaleDateString()}</li>
              </ul>
            </div>
            
            <div class="benefits-box">
              <h3>🌟 Your Enhanced Benefits</h3>
              <p>As an annual member, you'll enjoy:</p>
              <ul>
                <li>10% discount on all event tickets</li>
                <li>Exclusive access to member-only events</li>
                <li>Private WhatsApp group access</li>
                <li>Special perks from partner brands</li>
                <li><strong>2 months free</strong> compared to monthly payments!</li>
              </ul>
            </div>
            
            <p>No action is required from you - everything will happen automatically. Your membership will seamlessly transition from monthly to annual on ${new Date(monthlyExpiryDate).toLocaleDateString()}.</p>
            
            <div style="text-align: center; margin: 30px 0;">
              <a href="${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/member-dashboard" class="cta-button">View Your Dashboard</a>
            </div>
            
            <p>Thank you for choosing annual membership! We're excited to have you as a long-term part of the Eklektik AF family.</p>
            
            <p>If you have any questions about your upgrade, feel free to reach out to us at <a href="mailto:hello@eklektikmama.com">hello@eklektikmama.com</a>.</p>
    ...
    `,
      text_content: `
        Membership Upgrade Confirmed - Welcome to Annual!
        
        Hi ${firstName},
        
        Great news! Your membership upgrade to annual has been successfully processed. You're now set to enjoy even more savings and benefits!
        
        UPGRADE DETAILS:
        - Current Status: Monthly membership active until ${new Date(monthlyExpiryDate).toLocaleDateString()}
        - Annual Membership Starts: ${new Date(annualStartDate).toLocaleDateString()}
        - Savings: You'll save 2 months worth of membership fees!
        
        WHAT HAPPENS NEXT:
        - Right now: Continue enjoying your monthly membership benefits
        - On ${new Date(monthlyExpiryDate).toLocaleDateString()}: Your annual membership automatically begins
        - Next payment: ${new Date(new Date(annualStartDate).getTime() + 365 * 24 * 60 * 60 * 1000).toLocaleDateString()}
        
        YOUR ENHANCED BENEFITS:
        As an annual member, you'll enjoy:
        - 10% discount on all event tickets
        - Exclusive access to member-only events
        - Private WhatsApp group access
        - Special perks from partner brands
        ...
        No action is required from you - everything will happen automatically. Your membership will seamlessly transition from monthly to annual on ${new Date(monthlyExpiryDate).toLocaleDateString()}.
        
        Thank you for choosing annual membership! We're excited to have you as a long-term part of the Eklektik AF family.
        
        If you have any questions about your upgrade, feel free to reach out to us at hello@eklektikmama.com.
    ...
    const result = await sendEmailViaService(email, emailContent);
    console.log('Upgrade confirmation email sent:', result.messageId);
    return result;
